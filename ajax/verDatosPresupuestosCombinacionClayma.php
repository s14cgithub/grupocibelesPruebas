...
if(isset($_POST["accion"])&$_POST["accion"]=="verDatosPresupuestosCombinados")
{
	...
	$irpf = 0; //esto solo es para que funcione, hay que ponerlo para poner el valor correcto; y en clayma tambien
	...
	$resultado2=verEstadoFacturacionFinMesClayma($conexion);	
	...
	$prefactura = 0;
	$resultado1 = insertarFacturaClayma($conexion, 'Comb: '.$presupuestos, $nombreCliente,$idCliente, $fecha, $pedido, $cantidad, $formaPago,$cuentaBancaria, $campana, $detallada, $precioNeto, $iva,$irpf, $precioTotal, $provision, $aPagar, $inicialComercial,$anioSeleccionado,$combinadoSumatorio,$prefactura);
	...
	if(strpos( $resultado1, "Error" ) === 0 or count($resultado1)>0)
	{
		...
		while ($contador<count($presupuestos2) && $seguir == true)
		{
			...
			$resultado4 = copiarTemporalPreFacturaAFacturaDetalleClayma($conexion,$numFactura,$presupuestos2[$contador],$idEmpleado,$anioSeleccionado,1,$campana);
