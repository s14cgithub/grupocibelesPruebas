...
function buscarFactura()
{
	...
	listadoAbonosPendientes();
	
}

function listadoAbonosPendientes() //js_facturasSinCobrar
{	
	peticionUnica1=crearComunicacion(peticionUnica1);

	if(peticionUnica1)
	{							
		peticionUnica1.onreadystatechange = mostrarListadoAbonosPendientes;
		peticionUnica1.open("POST","ajax/mostrarAbonosVisualizar.php",false);
		peticionUnica1.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");		
		var query_string = consultaListadoAbonosPendientes();
		peticionUnica1.send(query_string);
	}
}

function consultaListadoAbonosPendientes()
{	
...
function mostrarListadoAbonosPendientes()
{
		...
		if(peticionUnica1.status == 200)
		{
			...
			{				
				...
				{  //Number(n).toLocaleString('es');
					...
					contenido += '<td><input type="image" id="'+datos[contador]["numero"]+'_modificar" value="" src="imagenes/modificar.png" style="width:15px;"  onclick="modificarAbonoPendiente('+datos[contador]["numero"]+')"></td>';
