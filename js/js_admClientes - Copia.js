		...
		if(peticionUnica1.status == 200)
		{
			...
			else
			{
				...
				while  (contador<datos.length)
				{
					...
					contenido += '<td align="right">Apellidos</td><td><input type="text" id="'+datos[contador]["id"]+'_contactoApellidos" name="'+datos[contador]["id"]+'_contactoApellidos" style="width: 100%;" value="'+datos[contador]["apellidos"]+'"></input></td></tr>';
					...
					contenido +='<td></td><td colspan="5"><textarea style="width: 100%;">'+datos[contador]["comentario"]+'</textarea></td></tr>';
...
function crearCliente()//aqui controlar campos obligatorios
{
	...
	if (document.getElementById("tipoClienteCliente").checked==true)//es cliente
	{		
		...
		verInformacionCliente("nif",document.getElementById("cliente_Nif").value)
		if(error==true)
		{
			seguir = false;	
			mensaje += "\nEl Nif ya existe";
