...
if(isset($_POST["imprimirAccion"]) && $_POST["imprimirAccion"]=="imprimirInformeGastosAdicionales")
{
	$ruta = '/';
	...
	{
		if ($f['campo1']=='fechaFacturacion' && $f['operador']=='>=')
		{
			$fechaInicio = $f['valor'];
		}
		if ($f['campo1']=='fechaFacturacion' && $f['operador']=='<=')
	...
	$campos = ['subcliente','concepto','fechaFacturacion','precioUnitario','unidadesSumatorio','totalSumatorio'];
	$group = ['idCliente','subcliente','concepto','precioUnitario','fechaFacturacion'];

	$resConsulta = cargarFacturasEspeciales($conn, $bbddSql, $campos, $filtros, $filtrosOperadores, $group, $order);
	...
	$imprimirTitulos=0;
	nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial, $fechaInicio, $fechaFin,$imprimirTitulos);
	$pdf->AliasNbPages();
	...
	$pdf->Cell(0,0,"BUROFAX",0,1,'C',false);
	...
	$nombreClienteAntiguo = "asdfjasdkfjk 234823498 !23j4kasdñfkasjf ";
	...
	{		
		$nombreCliente = $datosCertificados[$contador]["subcliente"];
		if ($nombreClienteAntiguo != $nombreCliente)
		{
			if ($nombreClienteAntiguo!="asdfjasdkfjk 234823498 !23j4kasdñfkasjf ")
			{
				$altura += 10;
				$pdf->SetFont('Arial','B',10);
				$pdf->SetXY($margen,$altura);
				$pdf->Cell(0,0,"Total: ".number_format($total,2,',','.')." ".EURO,0,1,'R',false); 
			...
			$total=0.00;
			$nombreClienteAntiguo = $nombreCliente;
		}
		
		
		if ($altura>250) 
		{			
			nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial, $fechaInicio, $fechaFin);	
		}
		$margen = $margenInicial;
	
		//echo $datosCertificados[$contador]["fechaFacturacion"]["date"];
		$altura += 10;
		$pdf->SetFont('Arial','',10);
		$pdf->SetXY($margen,$altura);
		$pdf->Cell(20,0,$datosCertificados[$contador]["fechaFacturacion"]->format('d/m/Y'),0,1,'R',false);
		...
		$margen+=30;
		$pdf->SetXY($margen,$altura);
		$pdf->Cell(0,0,utf8_decode($datosCertificados[$contador]["concepto"]),0,1,'L',false);
		$margen+=50;
		$pdf->SetXY($margen,$altura);
		$pdf->Cell(30,0,number_format($datosCertificados[$contador]["precioUnitario"],4,',','.')." ".EURO,0,1,'R',false);
		$margen = $margenInicial;
		$pdf->SetXY($margen,$altura);
		$pdf->Cell(0,0,number_format($datosCertificados[$contador]["totalSumatorio"],2,',','.')." ".EURO,0,1,'R',false);

		//$total += $datosCertificados[$contador]["totalSumatorio"];
	...
	if (count($datosCertificados)<=0)
	{
		$altura += 10;
		$pdf->SetFont('Arial','B',10);
		$pdf->SetXY($margen,$altura);
		$pdf->Cell(0,0,"No hay registros",0,1,'R',false); 
	}
	else
	{
...
function nuevaPagina(&$pdf,&$altura,$alturaSiguientePagina,$margenInicial, $fechaInicio, $fechaFin,$imprimirTitulos=1)
{
	$pdf->AddPage();
	//$pdf->SetAutoPageBreak(false);
	$margen = $margenInicial;
