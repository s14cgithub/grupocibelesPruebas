...
function duplicarPdf($numFactura1,$valor,$datosDetalles)
{
	...
	$pdfEmail->SetDrawColor(colorAzulR,colorAzulG,colorAzulB);
		...
		$pdfEmail->Cell(0,5,utf8_decode("Manipulación y envio de su Correspondencia diario del"),0,1,'L',false);
		
		$margen = 110;
		$pdfEmail->SetFont('Arial','B',9);
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(0,5,utf8_decode($laFechaInicio),0,1,'L',false);
		
		$margen = 130;
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->SetFont('Arial','',9);
		$pdfEmail->Cell(0,5,utf8_decode("al"),0,1,'L',false);
		
		$margen = 135;
		$pdfEmail->SetFont('Arial','B',9);
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(0,5,utf8_decode($laFechaFin),0,1,'L',false);
		...
		$pdfEmail->SetFont('Arial','B',10);
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(0,5,"CONCEPTOS SUJETOS A I.V.A.",0,1,'L',false);
		
		///////////////////////////////
		
		$pdfEmail->SetFont('Arial','B',9);
				
		$margen = $pdfEmail->GetPageWidth()-40;
		//$altura += 15;

		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(0,5,utf8_decode("Importe"),0,1,'R',false);

		$margen -= 20;
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(20,5,utf8_decode("Precio"),0,1,'R',false);

		$margen -= 20;
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(20,5,utf8_decode("Unidades"),0,1,'R',false);		
		...
		/*$margen += 120;
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(20,5,"Euros",0,1,'R',false);*/
		
		$altura += 10;
		
		
		$contadorConceptos++;
		dibujarContraste($pdfEmail,$margenInicial, $altura, $contadorConceptos);
		...
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->SetFont('Arial','',9);
		$pdfEmail->Cell(0,5,utf8_decode("Fijo Mensual"),0,1,'L',false);
		
		//$margen += 110;
		$margen = $pdfEmail->GetPageWidth()-40;
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(20,5,utf8_decode($valor["fac_cuotaRecogida"])." ".EURO,0,1,'R',false);
		...
		$margen -= 20;
		$pdfEmail->SetXY($margen,$altura);
		$pdfEmail->Cell(20,5,"1",0,1,'R',false);
		...
		{			
			...
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(0,5,utf8_decode("Manipulación Postal"),0,1,'L',false);

			$totalManipulado = 0.00;
			$totalManipulado = number_format($valor["cantidad"]*$valor["fac_cobroUnitarioEnvio"],2,',','.');
			$margen = $pdfEmail->GetPageWidth()-40;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,$totalManipulado." ".EURO,0,1,'R',false);

			$margen -= 20;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,number_format($valor["fac_cobroUnitarioEnvio"],3,',','.')." ".EURO,0,1,'R',false);

			$margen -= 20;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,number_format($valor["cantidad"],0,',','.'),0,1,'R',false);		
		...
		if ($valor["importeFranqueo"]!="" && $valor["importeFranqueo"]!=null && $valor["importeFranqueo"]!="null" && $valor["importeFranqueo"]>0 && $valor["fac_porCientoNoBonificable"]!="" && $valor["fac_porCientoNoBonificable"]!=null && $valor["fac_porCientoNoBonificable"]!="null" && $valor["fac_porCientoNoBonificable"]>0)
		{
			...
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(0,5,utf8_decode("Manipulado de Productos No Bonificables"),0,1,'L',false);
			...
			//$pdfEmail->Cell(20,5,$valor["importeFranqueo"],0,1,'R',false);

			$margen = $pdfEmail->GetPageWidth()-40;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,$tantoPorCiento." ".EURO,0,1,'R',false);

			$margen -= 20;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,$elPrecioFranqueo." ".EURO,0,1,'R',false);

			$margen -= 20;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,$valor["fac_porCientoNoBonificable"]." %",0,1,'R',false);
		...
		foreach ($datosDetalles as $valorDetalle)
		{
			...
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(0,5,utf8_decode($valorDetalle["concepto"]),0,1,'L',false);	
			
			$margen = $pdfEmail->GetPageWidth()-40;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(0,5,number_format($valorDetalle["total"],2,',','.')." ".EURO,0,1,'R',false);		
			$margen -= 20;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,number_format($valorDetalle["precio"],3,',','.')." ".EURO,0,1,'R',false);		
			$margen -= 20;
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(20,5,number_format($valorDetalle["unidades"],3,',','.'),0,1,'R',false);			
		...
		if ($valor["fac_otrosConceptosFijos"]!="" && $valor["fac_otrosConceptosFijos"]!=null && $valor["fac_otrosConceptosFijos"]!="null")
		{
			...
			$pdfEmail->SetXY($margen,$altura);
			$pdfEmail->Cell(0,5,utf8_decode($valor["fac_otrosConceptosFijos"]),0,1,'L',false);
