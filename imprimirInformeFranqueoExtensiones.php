...
if(isset($_POST["imprimirAccion"]) && $_POST["imprimirAccion"]=="imprimirFranqueo")
{
	...
	$idCliente = $_POST["numeroClienteExtension_form"];	
	$fechaInicio = $_POST["fechaInicioExtension_form"];	
	$fechaFin = $_POST["fechaFinExtension_form"];
	
	$separarPorFechas = "true";
	$saldo = "false";
	$sinIva="false";
	...
	{
		$datosFranqueo = verConsumoFranqueoGrabadosPorClienteYfechas($conexion,$idCliente,$fechaInicio,$fechaFin,'',1);
		//echo  verConsumoFranqueoGrabadosPorClienteYfechas($conexion,$idCliente,$fechaInicio,$fechaFin,'',1);
	}	
	...
	if (count($datosFranqueo)>0)
	{
		...
		$extensionAntiguo="asdjfaksdjfk238492834usdjfasf";
		...
		$resumen = 	verConsumoFranqueoExtensionResumen($conexion,$idCliente,$fechaInicio,$fechaFin);
		{
			//nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial);
			$contador=0;
			$altura=15;
			...
			$altura += 10;
			$totalTodoExtension = 0.00;
			while ($contador< count($resumen))
			{
				...
				$laExtension = $resumen[$contador]["ot"];
				
				$importeExtension = $resumen[$contador]["importeTotal"];

				//$pdf->Cell(0,0,utf8_decode($laExtension),0,1,'L',false);
				...
				$margen = 50;
				$pdf->SetXY($margen,$altura);
				$pdf->Cell(0,0,number_format($importeExtension,2,',','.')." ".EURO,0,1,'R',false);
				...
				$totalTodoExtension += $importeExtension;
			...
			$altura += 10;
				$margen = $margenInicial;
				$pdf->SetXY($margen,$altura);
				$pdf->Cell(0,0,utf8_decode("TOTAL: "),0,1,'L',false);
				
				$margen = 50;
				$pdf->SetXY($margen,$altura);
				$pdf->Cell(0,0,number_format($totalTodoExtension,2,',','.')." ".EURO,0,1,'R',false);
		...
		while ($contador<count($datosFranqueo))
		{
			...
			$nombreExtension = $datosFranqueo[$contador]["ot"];			
			
			
			if ($clienteAntiguo!=$nombreCliente || $extensionAntiguo!=$nombreExtension)
			{
				$fechaAntigua="asdjfaksdjfk238492834usdjfasf";
				...
				$extensionAntiguo = $nombreExtension;
				$pdf->Cell(0,0,utf8_decode($nombreCliente),0,1,'L',false);
				
				$altura += 10;
				$pdf->SetXY($margen,$altura);
				$pdf->Cell(0,0,utf8_decode($nombreExtension),0,1,'L',false);
			...
			//if (trim($datosFranqueo[$contador]["ot"])!=$ot)
			{
				...
				$unidadesExtension=0.00;
				$totalExtension=0.00;
				//$ot = trim($datosFranqueo[$contador]["ot"]); aqui
				$altura += 10;
				$pdf->SetTextColor(0,0,0);
				$pdf->SetFont('Arial','B',10);
				$pdf->SetXY($margen,$altura);
