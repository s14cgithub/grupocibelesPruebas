...
session_start(); 
$numFacturaOriginal = $_POST["facRecSustitucion_numeroFactura"];
$_SESSION['titulo']="FACTURA RECTIFICATIVA POR SUSTITUCION - ".$numFacturaOriginal;
...
if ($_SESSION["usuario"]!="" && isset($_POST["facRecSustitucion_Accion"]) && $_POST["facRecSustitucion_Accion"]=="verRecSustitucion")
{
	
	
	$origenFactura = isset($_POST["facRecSustitucion_OrigenFac"]) ? $_POST["facRecSustitucion_OrigenFac"] : 'FACTURA NORMAL';
	
	$clayma = $_POST["facRecSustitucion_clayma"];
	$claymaChecked = "";
	if ($clayma=="true")
	{
		$esClayma = "Sí";
		$claymaChecked = "checked";
		if ($origenFactura=="FACTURA NORMAL") //sin comprobar
		{
			$datosFacturaOriginal =  verFactura_numeroCompletoClayma($conexion,$numFacturaOriginal);
		}
		else if ($origenFactura=="facturaRectificativa") //sin hacer
		{
	...
	$imprimirDetalladaChecked = "";
	if ($detalladaImprimir==true)
	{
		$imprimirDetalladaChecked = "checked";		
	}
	...
	echo '<tr><td align="center" colspan="7" style="color:red";>
	<b><u>LA FACTURA ' . $numFacturaOriginal . ' SERÁ ANULADA</u></b>
	<hr></td></tr>';
	...
	echo '<td id="fechaFacturaOriginal">'.$datosFacturaOriginal[0]["fecha"]->format("Y-m-d").'</td>';
	...
	$valorPresupuesto = isset($datosFacturaOriginal[0]["presupuesto"]) ? $datosFacturaOriginal[0]["presupuesto"]: 0; //las rectificativas no tienen presupuesto guardado. 
	

	echo '<td id="presupuesto">'.$valorPresupuesto.'</td>';
	...
	echo '<tr><td align="right">Motivo:</td><td colspan="5"><input type="text" id="motivo" style="width:100%" value="" maxlength="200"></input></td></tr>';
	/*echo '<tr>';
	...
	echo '<tr>';
	echo '<td align="right">Clayma:</td>';
	//echo '<td  align="left" colspan="5"><span id="camposClayma"><input type="checkbox" id="clienteOrigen" name="clienteOrigen" value="" onchange="gestionClientes();" '.$claymaChecked.'></input> </span></td>';	
	echo '<td align="left">'. $esClayma.'</td>';	
	echo '</tr>';
	...
	*/
	...
	echo '<tr><td colspan="1" align="right">Cliente:</td>';
	echo '<td colspan="5"><select name="clientes" id="clientes" onchange="cargarPresupuestoYprovision()">';
	echo '</select></td></tr>';

	echo '<tr><td colspan="1" align="right">Presupuesto:</td>';
	echo '<td colspan="5"><select name="presupuestoCampo" id="presupuestoCampo" onchange="verProvisionTotalPresupuesto()">';
	echo '</select></td></tr>';
	...
	echo '<td colspan="5"><input type="text" id="pedidoCliente" style="width:100%">'.$datosFacturaOriginal[0]["pedido"].'</input></td></tr>';
	...
	echo '<td colspan="5"><select name="formaPago" id="formaPago"></select>';
	echo '&nbsp;<button type="button" class="btn btn-info" data-toggle="modal" data-target="#NuevaFormaPagoProcesoModal" data-whatever="@mdo"  style="height: =5px;line-height: 5px;">NFP</button></td>';
	echo '</tr>';

	echo '<tr><td align="right">Campaña:</td>';	
	echo '<td colspan="5"><input type="text" id="campana" value = "'.$datosFacturaOriginal[0]["descripcion"].'" style="width:100%"></input></td></tr>';



	echo '<tr><td align="right">Imprimir Detallada:</td>';
	echo '<td align="left"><input type="checkbox" id="detallada" name="detallada" value="detallada" '.$imprimirDetalladaChecked.'></td></tr>';
	...
	echo '<button type="button" class="btn btn-info" id="botonModificarPresupuesto" onClick="grabarFacturaRecSustitucion()">EMITIR FACTURA RECTIFICATIVA</button>';	
...
<form id="formPrevisualizarFacturaClayma"  method="post"  target="_blank" action="previsualizarFacturaClayma.php">
	<input type="hidden" id="previsualizarClayma_facturaRectificativa" name="previsualizarClayma_facturaRectificativa" value="BORRADOR por Sustitucion"></input>
	<input type="hidden" id="previsualizarClayma_presupuesto" name="previsualizarClayma_presupuesto" value=""></input>
	
	<input type="hidden" id="previsualizarClayma_facRecMotivo" name="previsualizarClayma_facRecMotivo" value=""></input>
	<input type="hidden" id="previsualizarClayma_facRecfacOriginal" name="previsualizarClayma_facRecfacOriginal" value=""></input>	
...
<form id="formPrevisualizarFactura"  method="post"  target="_blank" action="previsualizarFactura.php">
	<input type="hidden" id="previsualizar_facturaRectificativa" name="previsualizar_facturaRectificativa" value="BORRADOR por Sustitucion"></input>
...
<script  src="js/js_crearFacRecSustitucion.js?<?php echo (versionCibeles); ?>" type="text/javascript" language="JavaScript" charset="UTF-8"></script>
...
document.getElementById("clientes").value = <?php echo $datosFacturaOriginal[0]["codigo_saldo"]; ?>;
...
borrarTemporalDetallesRec();
copiarDetallesATemporalDetallesRec();
