...
if(isset($_POST["imprimirAccion"]) && $_POST["imprimirAccion"]=="imprimirFactura")
{
	...
	$pdf->SetFont('Arial','B',12);
	$pdf->SetXY($margen,$altura);
	$pdf->MultiCell(0,4,utf8_decode($datosFactura[0]["descripcion"]),0,'C',false);
	$anchoDescripcion = $pdf->GetStringWidth(utf8_decode($datosFactura[0]["descripcion"]));
	$numeroDeFilas = ceil ($anchoDescripcion / 180);
	if ($numeroDeFilas<1)
	{
		$numeroDeFilas = 1;
	}	
	$altura = $altura + (8*$numeroDeFilas);
