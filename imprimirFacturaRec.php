...
if(isset($_POST["imprimirAccion"]) && $_POST["imprimirAccion"]=="imprimirFacturaRec")
{
	$ruta = '/';
	...
	$numeroCompletoFacRec = isset($_POST["imprimirNumFacturaRec"]) ? $_POST["imprimirNumFacturaRec"] : 0;

	$partesFactura = explode("/",$numeroCompletoFacRec); //RECT 150/25
	$anioDosDigitos = $partesFactura[1];	
	$anioSeleccionado = $anioDosDigitos + 2000;

	$partesFactura1 = explode(" ",$partesFactura[0]);
	$tipoFactura = trim($partesFactura1[0]);
	$numFactura = trim($partesFactura1[1]);
	

	if($tipoFactura =="SUST")	
	{
		$tituloFac = "Factura Rectificativa por Sustitucion";
		$datosFactura = verFacturaRectificativaSustitucion($conexion,$numeroCompletoFacRec,$anioSeleccionado);	
		$datosDetalles = verFacturaRecSustDetalle($conexion,$numeroCompletoFacRec,$anioSeleccionado);
		
	}
	else
	{
		$tituloFac = "Factura Rectificativa por Diferencia";
		$datosFactura = verFacturaRectificativa($conexion,$numeroCompletoFacRec,$anioSeleccionado);	
		$datosDetalles = verFacturaRecDetalle($conexion,$numeroCompletoFacRec,$anioSeleccionado);
	}	
	...
	$datosNuevaPagina1 = $numeroCompletoFacRec;	
	...
	$datosCliente =  cargarClientes($conexion," where codigo_saldo=".$datosFactura[0]["idCodigoCliente"]." and codigo=".$datosFactura[0]["idCodigoCliente"]);
	...
	nuevaPagina($pdf,$altura,$alturaSiguientePagina,$datosNuevaPagina1,$datosNuevaPagina2,$datosFactura,$retener,$margenInicial,$datosPrefactura,$tituloFac);	           
...
Cibeles Mailing S.A. A-81339186"),0,'C',false);*/
	...
	*/
	...
	{	
		//$pdf->SetXY($ancho,$altura);			
		//$pdf->Cell(20,5,"ccccccccccccccccccccccccccccccccc",0,0,'R',false);
		// -------- QR --------
	...
	{ 
		//$pdf->SetXY($ancho,$altura);			
		//$pdf->Cell(20,5,"bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",0,0,'R',false);
...
//////////////////////////
	...
	$pdf->Output("I",$tituloFac." - ".$numFactura."-".date("dmy")." .pdf","UTF-8");
...
function nuevaPagina(&$pdf,&$altura,$alturaSiguientePagina,$datosNuevaPagina1,$datosNuevaPagina2,$datosFactura,$retener,$margenInicial,&$datosPrefactura,$tituloFac)
{
	$pdf->AddPage();
	...
	$pdf->SetXY($margen,$altura);
	$pdf->SetFont('Arial','',10);
	$pdf->Cell(35,5,utf8_decode("Rectifica a:"),1,1,'C',false);
	$altura = $altura + 5;
	$pdf->SetXY($margen,$altura);	
	...
	$altura = $altura + 8;
	$pdf->SetXY($margen,$altura);
	$pdf->Cell(35,5,$datosNuevaPagina2,0,1,'R',false);
	...
	$pdf->Cell(0,0,$tituloFac,0,1,'L',false);
