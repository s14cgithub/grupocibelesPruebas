...
if(isset($_POST["imprimirAccion"]) && $_POST["imprimirAccion"]=="imprimirInforme")
{
	...
	if ($fechaInicio != '01-01-1970' && $fechaInicio != '' && $fechaInicio != null && $fechaInicio != 'null' && $fechaFin != '01-01-1970' && $fechaFin != '' && $fechaFin != null && $fechaFin != 'null')
	{
		$pdf->Cell(0,0, "Informe del ".date("d-m-Y", strtotime($fechaInicio)). " al ".date("d-m-Y", strtotime($fechaFin)),0,1,'R',false);
	}
	else if ($fechaInicio != '01-01-1970' && $fechaInicio != '' && $fechaInicio != null && $fechaInicio != 'null')
	{
		$pdf->Cell(0,0, "Informe desde ".date("d-m-Y", strtotime($fechaInicio)),0,1,'R',false);
	}
	else if ($fechaFin != '01-01-1970' && $fechaFin != '' && $fechaFin != null && $fechaFin != 'null')
	{
		$pdf->Cell(0,0, "Informe hasta ".date("d-m-Y", strtotime($fechaFin)),0,1,'R',false);
	}
	...
	{		
		...
		{
			if ($altura>263)//260
			{	//$pdf->Cell(0,0,"alturaA: ".$altura,0,1,'C',false);	
				nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial);
			}
			if (!$primeraVez)
			{
				if ($altura>263)//252
				{//$pdf->Cell(0,0,"alturaB: ".$altura,0,1,'C',false);					
					nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial);
				}
				...
				if ($datosFactura[$contador-1]["saldo"]<0)
				{					
				...
				$margen = 60;
				$pdf->SetXY($margen,$altura);
				$pdf->Cell(0,0,"Saldo:   ".number_format($datosFactura[$contador-1]["saldo"],2,',','.')." ".EURO,0,1,'C',false);
			...
			if ($altura>240)//250
			{	
				//$pdf->Cell(0,0,"alturaC: ".$altura,0,1,'C',false);			
				nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial);
			}
		...
		if ($altura>260)
		{	//$pdf->Cell(0,0,"alturaD: ".$altura,0,1,'C',false);		
			nuevaPagina($pdf,$altura,$alturaSiguientePagina,$margenInicial);
		}
		...
		$pdf->MultiCell(70, 5, utf8_decode($datosFactura[$contador]["descripcion"]),0, 'L', 0);
		
		$anchoDescripcion = $pdf->GetStringWidth(utf8_decode($datosFactura[$contador]["descripcion"]));
		
		
		
		$numeroDeFilas = ceil($anchoDescripcion / 65);
